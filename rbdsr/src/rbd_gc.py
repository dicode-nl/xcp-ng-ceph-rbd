#!/usr/bin/python3
#
# rbd_gc.py - detached background garbage-collector for the Ceph RBD SR (SMAPIv1).
#
# VDI.delete renames an object with CoW children to "xcp-trash-<uuid>" (SR.scan
# ignores it) and spawns this worker, which flattens the children then purges the
# trashed object. If we die/defer, the trash object lingers (scan-invisible);
# SR.scan re-spawns us for stale jobs.
#
