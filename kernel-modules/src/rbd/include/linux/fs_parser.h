...
/* compat: fs_parse(fc, ...) and fs_lookup_param() dropped — they need the
 * full struct fs_context, which we do not backport. ceph/rbd use __fs_parse()
 * directly with a struct p_log, so this is sufficient. */
