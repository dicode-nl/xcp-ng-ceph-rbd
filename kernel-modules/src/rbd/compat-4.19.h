...
/* ===================== block layer (rbd only) ===================== */
#include <linux/device.h>

/* 4.19 has BUS_ATTR_RO/RW but not BUS_ATTR_WO */
#ifndef BUS_ATTR_WO
#define BUS_ATTR_WO(_name) \
	struct bus_attribute bus_attr_##_name = __ATTR_WO(_name)
#endif

/* lockdep_assert_held_write() added in 5.x */
#ifndef lockdep_assert_held_write
#define lockdep_assert_held_write(l) lockdep_assert_held(l)
#endif

/* blk_mq_alloc_disk()/blk_cleanup_disk() added in 5.14 */
...
compat_blk_mq_alloc_disk(struct blk_mq_tag_set *set, void *queuedata)
{
	...
	if (IS_ERR(q))
		return ERR_CAST(q);
	/* minors are set by the caller (rbd) before add_disk, as on 5.14 */
	disk = alloc_disk(0);
	if (!disk) {
		blk_cleanup_queue(q);
...
#define blk_mq_alloc_disk(set, queuedata) \
	compat_blk_mq_alloc_disk((set), (queuedata))
...
/* set_capacity_and_notify() added in 5.10 — backport of the upstream logic,
 * incl. the udev RESIZE=1 uevent so live `rbd resize` is seen by userspace.
 * Uses GENHD_FL_UP (4.19) in place of the later disk_live() helper. */
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
#include <linux/kobject.h>
static inline bool set_capacity_and_notify(struct gendisk *disk, sector_t size)
{
	...
	/* Only notify for a user-visible, live disk. */
	if (!(disk->flags & GENHD_FL_UP))
		return false;

	pr_info("%s: detected capacity change from %lld to %lld\n",
		disk->disk_name, (long long)capacity, (long long)size);

	/* Historically no uevent for changes to/from an empty device. */
	if (!capacity || !size)
		return false;
